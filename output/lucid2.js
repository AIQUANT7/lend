import { Lucid, Blockfrost, Data, Constr, fromText, validatorToAddress, getAddressDetails } from "@lucid-evolution/lucid";
...
function lenderDatum(totalPT, rewardPerPT, lenders) {
    const entries = lenders.map(([pkhHex, [balance, rewardDebt]]) => new Constr(0, [fromText(pkhHex), new Constr(0, [balance, rewardDebt])]));
...
async function depositPT(amount) {
    const lucid = await intiLucid();
    lucid.selectWallet.fromSeed("nest wink neither undo oven labor nature olympic file mandate glass inner cart theme initial fancy glow water mutual flame swap budget reform cute");
    ...
    const d = Data.from(poolUtxo.datum);
    ...
    const tx = await lucid
        ...
        .collectFrom([ptUtxo])
        .addSigner(address)
        .pay.ToContract(scriptAddr, { kind: "inline", value: Data.to(newDatum) }, newAssets)
        .complete();
    // console.log("tx ", tx);
