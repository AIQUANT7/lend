...
const initializeRoadmap = async (lucid, preId, roadmapId, roadmapName, roadmapDescription, prePkh, preSkh, totalPlasticCredits, totalPlasticTokens, totalPlastic) => {
    try {
...
//
const getPubKeyHash = async (lucid) => {
...
//
async function deposit(lucid, depositAmount) {
    try {
        ...
        if (contractUTxOs.length === 0) {
            ...
            return lucid
                .newTx()
                .pay.ToContract(contractAddress, { kind: "inline", value: Data.to(buildLenderDatum(initialDatum)) }, {
        ...
        const userUtxo = userUtxos.find((utxo) => utxo.assets[ptAssetUnit] && utxo.assets[ptAssetUnit] >= depositAmount);
        if (!userUtxo) {
        ...
        return lucid
            ...
            .pay.ToContract(contractAddress, { kind: "inline", value: Data.to(buildLenderDatum(newDatum)) }, {
            ...contractUTxO.assets,
...
async function withdraw(lucid, withdrawAmount) {
    try {
        ...
        if (newDatum.totalPT > 0n && newDatum.lenders.length > 0) {
            tx.pay.ToContract(contractAddress, { kind: "inline", value: Data.to(buildLenderDatum(newDatum)) }, remainingAssets);
        }
...
async function redeemReward(lucid) {
    try {
        const pkh = await getPubKeyHash(lucid);
        const userAddress = await lucid.wallet().address();
        const contractUTxOs = await lucid.utxosAt(contractAddress);
...
async function FundPlastik(lucid, preId, roadmapId, soldPlasticCredit) {
    try {
        ...
        refiContractAddress, { kind: "inline", value: Data.to(updatedDatum) }, refiAssets)
...
// const tx = await deposit(lucid, 1000n);
// if (!tx) {
